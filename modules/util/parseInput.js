...
module.exports = (text, gcfg) => {
  ...
  const data = {
    valid: new RegExp(`^(\\${gcfg.commands.prefixes.join(`|\\`)})(?![^a-zA-Z0-9])[a-zA-Z0-9]+(?=\\s|$)`).test(input), // checks if the input starts with the command prefix, immediately followed by valid characters.
