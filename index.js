    ...
    bot.mainGuild.members.fetch({ user: m.author.id, cache: true }).then(member => {
        ...
        if(input.valid) {
            ...
            ob.Assets.fetchCommand(input.cmd).then(cmd => {
                ...
                if(cmd.metadata.flags['BOT_CHANNEL_ONLY'] && m.channel.type !== 'dm' && bot.cfg.channels.bot.some(id => [channel.id, channel.parentID].includes(id)) && (authlvl === 0 || cmd.metadata.flags['STRICT'])) {
                    checkMisuse('This command can only be used in DMs (Direct Messages) OR the #optibot channel.');
                } else
                if(cmd.metadata.flags['MOD_CHANNEL_ONLY'] && m.channel.type !== 'dm' && bot.cfg.channels.mod.some(id => [channel.id, channel.parentID].includes(id)) && (authlvl < 5 || cmd.metadata.flags['STRICT'])) {
